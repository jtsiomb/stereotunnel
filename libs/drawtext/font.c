...
int dtx_get_font_glyphmap_count(struct dtx_font *fnt)
{
	return fnt->num_gmaps;
}

struct dtx_glyphmap *dtx_get_font_glyphmap_idx(struct dtx_font *fnt, int idx)
{
	int i;
...
struct dtx_glyphmap *dtx_load_glyphmap(const char *fname)
{
	...
	gmap = dtx_load_glyphmap_asset(fp);
	ass_fclose(fp);
	return gmap;
}

struct dtx_glyphmap *dtx_load_glyphmap_asset(ass_file *fp)
{
	char buf[512];
	int hdr_lines = 0;
	struct dtx_glyphmap *gmap;
	...
	while(hdr_lines < 3) {
		char *line = buf;
		if(!ass_fgets(buf, sizeof buf, fp)) {
	...
	for(i=0; i<num_pixels; i++) {
		int c = ass_fgetc(fp);
		if(c == -1) {
			fprintf(stderr, "unexpected end of file while reading pixels (%d/%d)\n", i, num_pixels);
			goto err;
		}
		gmap->pixels[i] = 255 * c / max_pixval;
		ass_fseek(fp, 2, SEEK_CUR);
	}
